...
export async function startSession(req: Request, res: Response): Promise<any> {
  ...
  const client = SessionUtil.getClient(session) as any;
  const expired = SessionUtil.isQRCodeExpired(client);
  // Check if there is an active QR code and if it is expired
  let shouldForceNewQR = false;
  if (!client.qrcode || expired) {
    client.urlcode = null;
    client.qrTimestamp = null;
    shouldForceNewQR = true;
  }
  if (shouldForceNewQR) {
    await SessionUtil.opendata(req, session, waitQrCode ? res : null, shouldForceNewQR);
  } else {
