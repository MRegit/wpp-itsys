...
export default class CreateSessionUtil {
  ...
  isQRCodeExpired(client: any) {
    ...
    if (!client.qrTimestamp) {
      console.log('[isQRCodeExpired] No qrTimestamp, QR NO expirado');
      return false;
    }
    const diff = Date.now() - client.qrTimestamp;
    const expired = diff > QR_EXPIRATION_TIME;
    return expired;
  }
